...
export function appendLead(l: {
  ...
  city: string | null; vehicle: string | null; sitePhase: string | null;
  chargerInterest: string | null; status: string;
}) {
  return append('Sales Leads', [
    l.leadNumber, fmtDate(l.createdAt), l.name, `+${l.whatsappNumber}`,
    l.city, l.vehicle, l.sitePhase, l.chargerInterest, l.status,
  ]);
}

export function appendComplaint(c: {
  ticketNumber: string; createdAt: Date; name: string | null; whatsappNumber: string;
  city: string | null; chargerModel: string | null; issueType: string | null;
  status: string; attachmentUrl: string | null;
}) {
  return append('Complaints', [
    c.ticketNumber, fmtDate(c.createdAt), c.name, `+${c.whatsappNumber}`,
    c.city, c.chargerModel, c.issueType, c.status, c.attachmentUrl,
  ]);
}