...
export default function CompatibilityChecker() {
  ...
  const isOptimal =
    selectedVehicle &&
    selectedVehicle.maxAcPower >= chargerPower;
  ...
  function handleModelChange(value: string) {
    ...
    if (selected) {
      setYear(
        String(
          selected.years[selected.years.length - 1]
        )
      );
    }
  ...
  return (
    ...
    >
      <div className="grid lg:min-h-[650px] lg:grid-cols-2">
        ...
        <div className="flex min-h-[520px] flex-col border-b border-white/20 px-5 py-10 md:px-8 lg:min-h-[650px] lg:border-b-0 lg:border-r lg:px-12 lg:py-10">
          {/* TOP */}
          ...
          >
            <h2 className="max-w-[580px] text-[48px] font-medium leading-[0.92] tracking-normal sm:text-[54px] lg:text-[60px] xl:text-[64px]">
              Built for
              <br />
              your EV.
            </h2>

            <p className="mt-7 max-w-[450px] text-[16px] leading-[1.65] text-white/65">
              Select your vehicle to see whether this charger is
              compatible and the maximum charging rate your car can
              accept.
            </p>
          </motion.div>
        </div>
        ...
        <div className="flex min-h-[520px] items-center px-5 py-10 md:px-8 lg:min-h-[650px] lg:px-12 lg:py-10">
          <div className="w-full">
            ...
            <div className="mb-6 flex items-center justify-between">
              ...
              <p className="text-[10px] font-medium uppercase tracking-[0.06em] text-white/30">
                Demo database
              </p>
            </div>
            ...
            <button
              ...
              disabled={!canCheck}
              onClick={handleCheck}
              className="group mt-6 flex h-[64px] w-full items-center justify-between bg-accent px-6 text-brand transition-colors duration-300 hover:bg-accent-hover disabled:cursor-not-allowed disabled:opacity-50"
            >
              <span className="text-[14px] font-medium uppercase tracking-[0.04em]">
                Check Compatibility
              </span>
            ...
            <div className="mt-6 flex items-center justify-between border-t border-white/20 pt-4">
              <span className="text-[10px] font-semibold uppercase tracking-[0.06em] text-white/45">
                11 kW AC Charger
              </span>
            ...
            <AnimatePresence mode="wait">
              {checked && selectedVehicle && (
                <motion.div
                  ...
                  className="mt-8 border-t border-white/25 pt-7"
                >
                  {/* STATUS */}

                  <div className="flex flex-col justify-between gap-5 border-b border-white/20 pb-6 sm:flex-row sm:items-end">
                    <div>
                      <p className="text-[10px] font-semibold uppercase tracking-[0.07em] text-white/40">
                        Compatibility Result
                      </p>

                      <p className="mt-2 text-[24px] font-medium tracking-[-0.025em]">
                        {brand} {model} · {year}
                      </p>
                    </div>
                  ...
                  <div className="pt-6">
                    <p className="text-[10px] font-semibold uppercase tracking-[0.07em] text-accent">
                      {isOptimal
                        ? "Optimized Match"
                        : "Compatible Match"}
                    </p>

                    <p className="mt-3 max-w-[600px] text-[16px] leading-[1.6] text-white/70">
                      {isOptimal
...
function SelectRow({
  ...
  return (
    <div
      ref={wrapperRef}
      className={`relative grid min-h-[72px] grid-cols-[48px_100px_1fr] items-stretch border transition-colors duration-300 ${
        open
    ...
    >
      ...
      <div className="relative border-l border-white/15">
        <button
          ...
          className="group flex h-full min-h-[70px] w-full items-center justify-between px-5 text-left"
        >
          ...
          <span
            aria-hidden
            className={`flex h-8 w-8 items-center justify-center text-accent transition-transform duration-300 ${
              open ? "rotate-180" : ""
            }`}
          >
            <svg
...
function ResultStat({
  ...
  return (
    <div className="border-r border-t border-white/20 py-5 odd:pr-5 even:border-r-0 even:pl-7">
      <p className="text-[10px] font-semibold uppercase tracking-[0.06em] text-white/40">
        {label}
      </p>

      <p className="mt-2 text-[24px] font-medium tracking-[-0.03em]">
        {value}
      </p>
    </div>
  );
